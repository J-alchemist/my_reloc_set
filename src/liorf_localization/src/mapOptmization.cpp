...
public:
    ...
    {   
        ...
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("liorf_localization/mapping/cloud_registered_raw", 1);   // 原始
    ...
    {   
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // extractSurroundingKeyFrames();       // 可以改为全局地图切割
...
// #ifdef  ENABLE_ICP_INIT
    ...
    // void visualizeGlobalMapThread()         // 地图保存
    // {
    //     ros::Rate rate(0.2);
    //     while (ros::ok()){
    //         rate.sleep();
    ...
    // void extractNearby()        // 半径surroundingKeyframeSearchRadius内搜索关键帧
    // {
    ...
    // //   提取出当前帧的 surroundingKeyframeSearchRadius 范围内的点云，得到laserCloudSurfFromMapDS局部地图
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    // <!-- 111 -->
    void scan2MapOptimization()             // 这里可以改成 多特征地图，建立多种优化去做
    {
        if (cloudKeyPoses3D->points.empty())
            return;
...
int main(int argc, char** argv)
{
    ...
    // std::thread loopthread(&mapOptimization::loopClosureThread, &MO);        // 回环线程close
    // std::thread visualizeMapThread(&mapOptimization::visualizeGlobalMapThread, &MO);     // 回环可视化close
